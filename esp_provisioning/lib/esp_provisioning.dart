...
class EspProvisioning {
  ...
  Future<List<EspBleDevice>> scanForDevices(String? deviceNamePrefix) async {
    final deviceJsonStrings =
        await _espPlatform.scanForEspDevices(deviceNamePrefix);
    final devicesJson = deviceJsonStrings
  ...
  Future<void> connect(String deviceName, String provisioningServiceUuid,
      String? proofOfPossession) async {
    await _espPlatform.connectDevice(
  ...
  Future<void> disconnect(String deviceName) =>
      _espPlatform.disconnectDevice(deviceName);
  ...
  Future<List<EspWifiAccessPoint>> getAccessPoints(String deviceName) async {
    ...
    final accessPointJsonStrings =
        await _espPlatform.getEspAccessPoints(deviceName).timeout(
              const Duration(seconds: 10),
              onTimeout: () => throw TimeoutException(
                  'Failed to get access points within 10 seconds.'),
            );

    // Deserialize the JSON strings into maps.
    final accessPointMaps = accessPointJsonStrings
        .map((e) => json.decode(e) as Map<String, dynamic>)
        .toList();

    // Convert the maps into EspWifiAccessPoint objects and return the sorted by SSID.
    final accessPoints = accessPointMaps
        .map(EspWifiAccessPoint.fromJson)
        .toList()
  ...
  Future<String> sendDataToEndpoint(
          String deviceName, String endpoint, String base64Data) =>
